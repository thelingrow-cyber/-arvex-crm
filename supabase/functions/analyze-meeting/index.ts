...
async function authenticate(req: Request, admin: SupabaseClient, SERVICE_KEY: string) {
  const authHeader = req.headers.get("authorization") || req.headers.get("Authorization") || "";
  const token = authHeader.replace(/^Bearer\s+/i, "").trim();
  if (!token) return { ok: false as const, status: 401, msg: "não autorizado (sem token)" };
  if (token === SERVICE_KEY) return { ok: true as const, userId: null as string | null, isAdmin: true, isService: true };
  const { data, error } = await admin.auth.getUser(token);
  if (error || !data?.user) return { ok: false as const, status: 401, msg: "token inválido" };
  const userId = data.user.id;
  const { data: profile } = await admin.from("profiles").select("role").eq("id", userId).single();
  const isAdmin = profile?.role === "admin";
  return { ok: true as const, userId, isAdmin, isService: false };
}
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { data: meetingRow, error: mErr0 } = await admin
      .from("meetings").select("id, closer_id, transcript").eq("id", meeting_id).single();
    if (mErr0 || !meetingRow) return json({ error: "Reunião não encontrada" }, 404);

    if (!auth.isService && !auth.isAdmin && meetingRow.closer_id !== auth.userId) {
      return json({ error: "não autorizado (não é dono desta reunião)" }, 403);
    }
