...
class Transcriber:
    def __init__(
        ...
        # Vocabulary is a soft bias baked into the initial_prompt (same
        # mechanism as INITIAL_PROMPT_PT's punctuation nudge, AD-2) -- not a
        # hard dictionary. Whisper has no hard-constraint decoding mode; this
        # just makes the listed terms likelier without guaranteeing them.
        if custom_vocabulary:
            terms = ", ".join(custom_vocabulary)
            self.initial_prompt = f"{INITIAL_PROMPT_PT} Termos e nomes usados com frequência: {terms}."
        else:
            self.initial_prompt = INITIAL_PROMPT_PT
