...
-- ⚠️ A policy legada "profiles_self" (FOR ALL, permissive, with check id = auth.uid())
-- ANULAVA a trava abaixo: policies permissivas se combinam com OR, então bastava passar
-- por ela para gravar role='admin'. Dropar é obrigatório — insert/select/update têm
-- policies próprias abaixo e o app nunca deleta profiles.
