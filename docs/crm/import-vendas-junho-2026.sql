...
begin
  ...
  if exists (select 1 from vendas where cliente = p_cliente and valor_total = p_valor) then
    raise notice 'Pulando % (venda já existe)', p_cliente;
    return;
  end if;
...
select _imp_venda_mensal('Roberta',         4997.00, 'Mentoria',    'parcelado',  'cartao', 12,  date '2026-06-08', 'cliente paga 12x R$516 no cartão (c/ juros)');
...
select _imp_venda_mensal('Tamires',         4500.00, 'Mentoria',    'parcelado',  'cartao', 10,  date '2026-06-20', 'PLACEHOLDER — valor/condição/data a confirmar com Vitor');
...
begin
  if exists (select 1 from vendas where cliente = 'Marília' and valor_total = 10000.00) then
    raise notice 'Pulando Marília (venda já existe)';
    return;
  end if;
  ...
  values (v_lead, 'Marília', 10000.00, 'Cindy Batista', 'entrada_saldo', 'Vitor',
          'Consultoria · 2x Pix (R$5.000 em 18/06 + R$5.000 em 19/06)')
  returning id into v_venda;
